...
* Download the latest `.war` file for the ART REST Component into the Tomcat server's `webapps` directory:
  * `sudo wget https://github.com/SmarterApp/TDS_AdministrationAndRegistrationTools/releases/download/2.1.0/testreg.rest-2.1.0.war -O /var/lib/tomcat7/webapps/rest.war`
* Download the latest `.war` file for the ART Web Application Component into the Tomcat server's `webapps` directory:
  * `sudo wget https://github.com/SmarterApp/TDS_AdministrationAndRegistrationTools/releases/download/2.1.0/testreg.webapp-2.1.0.war -O /var/lib/tomcat7/webapps/ROOT.war`
