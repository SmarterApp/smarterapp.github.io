...
* A SAML XML metadata file for the REST component, located where-ever the file name/path is configured for `pm.security.dir` and `pm.rest.saml.metadata.filename` (e.g. `/var/lib/tomcat7/resources/security/art_rest_local_sp.xml`)
* A SAML XML metadata file for the web application component located where-ever the file name/path is configured for `pm.security.dir` and `pm.webapp.saml.metadata.filename` (e.g. `/var/lib/tomcat7/resources/security/art_local_sp.xml`)
